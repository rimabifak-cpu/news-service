...
class Channel(Base):
    ...
    ai_prompt = Column(Text)  # Промт по умолчанию для канала
    logo_path = Column(String(512))
    logo_position = Column(String(50), default="bottom-right")
...
class Source(Base):
    ...
    channel_id = Column(Integer, ForeignKey("channels.id"), nullable=True)  # Связь с каналом
    is_active = Column(Boolean, default=True)
